    ...
    // Recover and verify
    {
        ...
        for (int i = 0; i < 5; ++i) {
            EXPECT_EQ("key" + std::to_string(i), entries[i].key) 
                << "Entry " << i << " key should match";
            EXPECT_EQ(static_cast<uint64_t>(i), entries[i].lsn()) 
                << "Entry " << i << " LSN should match";
            EXPECT_EQ(static_cast<uint64_t>(i * 100), entries[i].ts)
                << "Entry " << i << " timestamp should match";
        }
...
TEST_F(WALTest, PartialRead) {
    ...
    // Write entries
    {
        ...
        for (int i = 0; i < 10; ++i) {
            auto entry = createTestEntry("key" + std::to_string(i), 
    ...
    // Read partial entries
    {
        ...
        for (size_t i = 0; i < entries.size(); ++i) {
            ...
            EXPECT_EQ("key" + std::to_string(expected_lsn), entries[i].key)
                << "Entry key should match at index " << i;
            EXPECT_EQ(expected_lsn, entries[i].lsn())
                << "Entry LSN should match at index " << i;
        }
    }
}
