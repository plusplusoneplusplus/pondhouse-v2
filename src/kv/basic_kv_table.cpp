...
BasicKvTable::BasicKvTable(std::shared_ptr<common::IAppendOnlyFileSystem> fs,
                           const std::string& table_name,
                           std::size_t max_wal_size)
...
common::Result<void> BasicKvTable::Put(const Key& key, const Value& value, bool acquire_lock) {
...
common::Result<BasicKvTable::Value> BasicKvTable::Get(const Key& key, bool acquire_lock) const {
    std::optional<std::lock_guard<std::mutex>> lock;
    if (acquire_lock) {
        lock.emplace(mutex_);
    }
...
common::Result<void> BasicKvTable::Delete(const Key& key, bool acquire_lock) {
    using ReturnType = common::Result<void>;
    std::optional<std::lock_guard<std::mutex>> lock;
    if (acquire_lock) {
        lock.emplace(mutex_);
...
common::Result<std::shared_ptr<BasicKvTable::Iterator>> BasicKvTable::ScanPrefix(const std::string& prefix,
...
common::Result<void> BasicKvTable::BatchPut(const std::vector<std::pair<Key, Value>>& entries) {
...
common::Result<std::vector<common::Result<BasicKvTable::Value>>> BasicKvTable::BatchGet(
    const std::vector<Key>& keys) const {
    std::lock_guard<std::mutex> lock(mutex_);

    std::vector<common::Result<Value>> results;
    ...
    return common::Result<std::vector<common::Result<Value>>>::success(std::move(results));
}

common::Result<void> BasicKvTable::BatchDelete(const std::vector<Key>& keys) {
    using ReturnType = common::Result<void>;
    std::lock_guard<std::mutex> lock(mutex_);
...
common::Result<bool> BasicKvTable::Recover() {
    ...
    for (std::size_t sequence : metadata_state_machine_->GetActiveLogSequences()) {
...
common::Result<void> BasicKvTable::Flush() {
    std::lock_guard<std::mutex> lock(mutex_);
    return SwitchMemTable();
}

common::Result<common::LSN> BasicKvTable::WriteToWAL(KvEntry& entry) {
...
common::Result<void> BasicKvTable::SwitchMemTable() {
...
common::Result<void> BasicKvTable::RotateWAL() {
    using ReturnType = common::Result<void>;
...
std::string BasicKvTable::GetWALPath(std::size_t sequence_number) const {
    return table_name_ + "/" + std::to_string(sequence_number) + ".wal";
}

common::Result<void> BasicKvTable::TrackMetadataOp(MetadataOpType op_type, const std::vector<FileInfo>& files) {
    using ReturnType = common::Result<void>;
...
common::Result<std::shared_ptr<BasicKvTable::Iterator>> BasicKvTable::NewIterator(common::HybridTime read_time,
